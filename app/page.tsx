...
const NEON_CYCLE = ["neon-magenta", "neon-yellow", "neon-green", "neon-cyan"];

type TickerRow = { player: string; game: string; score: number; timeAgo: string };

function buildTickerRows(): TickerRow[] {
  return TIME_AGO.map((timeAgo, i) => {
    const game = GAMES[i % GAMES.length];
    const [row] = seededScores(game.id.length * 13 + i * 7 + 5, 1);
    return { player: row.name, game: game.title, score: row.score, timeAgo };
  });
}
...
export default function Home() {
  useReveal();
  const tickerRows = buildTickerRows();
  const topPlayers = seededScores(211, 5);
            ...
            <div className="ticker">
              {tickerRows.map((r, i) => (
                <div key={i} className="tick-row" style={{ animationDelay: i * 60 + "ms" }}>
                  <span className={"tk-p " + NEON_CYCLE[i % NEON_CYCLE.length]}>{r.player}</span>
